...
#         if self.verbose == 'yes':
#             print(f'ParallelRequester with {max_parallel_requests} parallelism')
#         self._max_parallel_requests = max_parallel_requests
#         if self.verbose == 'yes':
#             print(f'Generating credentials using {credentials_file}')
#         self._credentials = service_account.Credentials.from_service_account_file(credentials_file)
#         self._scoped_credentials = self._credentials.with_scopes(scopes)
...
#         }
#         if self.verbose == 'yes':
#             print('Beginning parallel requests')
#         self._responses = len(pending_requests) * [None]
#         for request_id, body in enumerate(pending_requests):
#             while threading.active_count() > self._max_parallel_requests:
#                 # Wait around for another thread to complete
...
#                 time.sleep(0.25)
#             if self.verbose == 'yes':    
#                 print('Thread is free, launching request')
#             new_thread = threading.Thread(target=self._single_request, args=(request_id, url, headers, body))
#             new_thread.start()
#         if self.verbose == 'yes':
#             print('Parallel requests complete, flattening results')
#         while threading.active_count() > 1:
#             # Wait around for threads to finish...
#             if self.verbose == 'yes':
#                 print(
...
#     def _single_request(self, request_id, url, headers, body):
#         if self.verbose == 'yes':
#             print(f'Making request for request id:{request_id}')
#         data = json.dumps(body).encode('utf-8')
#         #print(f'Request {request_id} body: {data}')
#         req = urllib.request.Request(url, data, headers)
#         resp = urllib.request.urlopen(req)
#         http_response = resp.read()
#         response = zlib.decompress(http_response, 16 + zlib.MAX_WBITS)
