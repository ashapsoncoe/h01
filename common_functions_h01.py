...
### All functions created by Alexander Shapson-Coe, except where indicated otherwise


class ParallelLocationRequester:

  ### Created by Tim Blakely, Google LLC
...
def get_density_of_values(input_data, plot_bandwidth, chosen_kernel, upper_lim_plot = None):

    if upper_lim_plot == None:
        upper_lim_plot = int(max(input_data)*1.25)

    xs = [[x] for x in np.linspace(0, upper_lim_plot,int(upper_lim_plot/plot_bandwidth))]
    density = KernelDensity(kernel=chosen_kernel, bandwidth=plot_bandwidth).fit([[x] for x in input_data])
    log_dens = density.score_samples(xs)

    return [x[0] for x in xs], np.exp(log_dens)


def plt_prob_dens_vs_dist(ax, ax2, fig, save_path, title, xs1, dens_vals1, ylab1, xs2=None, dens_vals2=None, ylab2=None):

    _ = ax.plot(xs1, dens_vals1, color="red", marker="o")
    _ = ax.set_xlabel("Distance from soma (nm)", fontsize = 14)
    _ = ax.set_ylabel(ylab1, color="red", fontsize=14)

    if xs2 != None:
        _ = ax2.plot(xs2, dens_vals2, color="blue", marker="o")
        _ = ax2.set_ylabel(ylab2, color="blue", fontsize=14)
...
def get_skel_data_from_shard_dir(this_batch_all_ids, shard_dir):
    ...
    c = [[[str(x[0]), str(x[1]), float(x[2]), float(x[3]), float(x[4]), float(x[5]), str(x[6])] for x in a] for a in c]
...
def make_one_skel_graph_ig(list_of_lists, skel_voxel_size, join_separate_components=True, join_by_end_nodes = False):
    ...
    for pos, sublist in enumerate(list_of_lists):

        num_nodes_prior = sum([len(x) for x in list_of_lists[:pos]])

        for edge in sublist:

            edge[0] = str(int(edge[0])+num_nodes_prior)

            if str(edge[6]) != '-1':
                edge[6] = str(int(edge[6])+num_nodes_prior)
...
def make_skel_graphs_batch(i, segs_to_load, skel_dir, skel_voxel_size, credentials_file, neurite_type, syn_db_name, synapse_voxel_size, syn_location_d=None, max_skel_components=100000):
    ...
    if syn_location_d == None:
        ...
        for r in results:
            ...
            if str(r['partner_seg_id']) == 'None':
                syn_location_d[own_id][combined_id]['partner_id'] = 'unidentifiedpartner' 
            else:
    ...
    for seg_id in seg_list:
        ...
        if len(raw_skel_dict[seg_id]) > max_skel_components:
            print(f'Skipped making graph for seg ID {seg_id}, as it has {len(raw_skel_dict[seg_id])} separate components')
            continue
        ...
        for node, syn_id, loc, partner in zip(chosen_nodes, syn_ids, syn_locations, syn_partners):
            ...
            if g.vs[node]['ntype'] == 's': # If synapse already assigned to this node:
                ...
                if node_locs == []:
                    #print(f'No non-syn nodes to assign syn to, seg node num is {len(list(g.vs))}, with {len(syn_ids)} synapses, seg_id {seg_id}')
                    #time.sleep(30)
                    continue
...
def in_circle(circle, x, y):

    # Checks if the point (x,y) is in the circle described by circle
...
def fix_layer_mem(bounds, all_point_data):
    ...
    mask = in_circle(bounds[0], xy[:,0], xy[:,1])
    ids = seg_ids[mask]
    coords = xy[mask]
    cluster_name = "White matter"
    cortical_layers[cluster_name] = list(ids)
    cortical_layers_coords[cluster_name] = coords
    classified_points = mask

    # get central layers
    for i in range(len(bounds) - 1):
        raw_mask = in_circle(bounds[i+1], xy[:,0], xy[:,1])
        # Don't reclassify points
        mask = np.logical_and(raw_mask, np.logical_not(classified_points))
        # Update classified points
        classified_points = np.logical_or(classified_points, mask)
    ...
    mask = np.logical_not(classified_points)
    ids = seg_ids[mask]
    coords = xy[mask]
...
def classify_syn_remove_stalks(g, clf):
    ...
    original_syn_nodes = [n.index for n in g.vs if 's' in n['ntype']]

    for syn_node in original_syn_nodes:

        assert g.vs[syn_node]['partner'] != None
        ...
        if prediction == 0:
            ...
            if root_node == syn_node:
                assert g.vs[syn_node]['ntype'] == 'sr'
                

            else:

                # Get rid of stalk nodes:
                for node in g.get_all_shortest_paths(syn_node, to=root_node, mode='OUT')[0]:
                    if g.vs[node]['ntype'] == 't':
                        g.vs[node]['ntype'] = 'c'

                g.vs[syn_node]['stalkdist'] = 0

                # Reassign to closest backbone node within 30 of root that is not already associated with a synapse, if one exists:
                    
                root_node = g.vs[syn_node]['rootnode']
                nodes_within_30_of_root = set([a for b in g.get_all_simple_paths(root_node, mode='OUT', cutoff=30) for a in b])
                bb_node_candidates = [x for x in nodes_within_30_of_root if g.vs[x]['ntype'] in ['b', 'r'] and x != syn_node]
                ...
                if bb_node_candidates != []:
                    ...
                    assert new_node != syn_node

                    for k in ['stalkdist', 'stype', 'synid', 'truex', 'truey', 'truez', 'rootnode', 'eucdistshaft', 'eucdistroot', 'partner']:
                    ...
                    for k in ['synid', 'truex', 'truey', 'truez', 'rootnode', 'eucdistshaft', 'eucdistroot', 'stalkdist', 'nhr', 'partner', 'stype']:
                        g.vs[syn_node][k] = None

                    g.vs[syn_node]['ntype'] = 'c'

    final_syn_nodes = [n.index for n in g.vs if 's' in n['ntype']]
    assert len(final_syn_nodes) == len(original_syn_nodes)
    final_partners = [str(g.vs[n]['partner']) for n in final_syn_nodes]
    #print('f', final_partners)
    #print('o', [str(g.vs[n]['partner']) for n in original_syn_nodes])
    assert 'None' not in final_partners
...
def get_dists_to_soma_from_xyz_points(syn_locations, g, soma_nodes, soma_target_type):

        all_nodes = [n.index for n in g.vs]

        closest_syn_nodes = get_skel_nodes_closest_to_synapses(syn_locations, g, all_nodes)
        ...
        return chosen_soma_nodes, shortest_paths, sp_dists, closest_syn_nodes
...
def get_partners_and_distances_to_soma_one_dir(
        ...
        save_dir, class_lookup, base_seg_lookup = None,
        save_ng_states = False, em = None, segmentation = None):
    ...
    if base_seg_lookup == None:
        base_seg_lookup = {x: [x] for x in seg_ids}
        seg_id_key = 'neuron_id'
    else:
        seg_id_key = 'base_neuron_id'

    segs_to_lookup = [str(a) for b in base_seg_lookup.values() for a in b]
        
    required_info = [
        f'{partner_side}.neuron_id AS partner_agglo_id',
        f'{partner_side}.id AS partner_syn_id',
        f'{own_side}.{seg_id_key} AS pr_cell_seg_id',
    ...
    results = get_info_from_bigquery(required_info, f'{own_side}.{seg_id_key}', segs_to_lookup, syn_db_name, client)

    organised_synapses = {x: [a for a in results if str(a['pr_cell_seg_id']) in base_seg_lookup[x]] for x in seg_ids}
    ...
    for pos, agglo_id in enumerate(seg_ids):
        ...
        chosen_soma_nodes, shortest_paths, sp_dists, closest_syn_nodes = get_dists_to_soma_from_xyz_points(syn_locations, g, soma_nodes, soma_target_type)
        
        selected_types = [get_neighbourhood_node_type(g, node, class_lookup) for node in closest_syn_nodes]
        ...
        for chosen_soma_node, min_dist, sel_type, syn_node, syn_loc, r in zipped:
            ...
            if sel_type not in self_structure: 
                print(sel_type)
                continue
            ...
            final_result_this_syn = {
                ...
                'pr_cell_seg_id': str(r['pr_cell_seg_id']),
        ...
        # If selected, make a neuroglancer state of the shortest paths for each synapse. Not reccomended for thousands of synapses
        if save_ng_states == True:

            syn_ids = [f"{x['pre_cell_syn_id']}_{x['partner_syn_id']}" for x in organised_synapses[agglo_id]]

            viewer = neuroglancer.Viewer()

            with viewer.txn() as s:

                dimensions = neuroglancer.CoordinateSpace(scales=[1,1,1], units='nm', names=['x', 'y', 'z'])

                s.showSlices = False
                s.dimensions = dimensions
                s.crossSectionScale = 1.0
                s.projectionScale = 80000
                s.position = np.array(syn_loc)

                s.layers['em'] = neuroglancer.ImageLayer(source = em)
                s.layers['segmentation'] = neuroglancer.SegmentationLayer(source = segmentation, segments=[agglo_id])
                s.layers['segmentation'].objectAlpha = 0.30 

                for shortest_path, sp_dist, syn_id in zip(shortest_paths, sp_dists, syn_ids):

                    pos = shortest_paths.index(shortest_path)

                    s.layers[f'{syn_id}, {sp_dist}nm'] = neuroglancer.AnnotationLayer()
                    s.layers[f'{syn_id}, {sp_dist}nm'].visible = False

                    for pos, edge in enumerate(shortest_path):
                        
                        source_node, target_node = edge

                        source_loc = [g.vs[source_node][a] for a in ('x', 'y', 'z')]
                        target_loc = [g.vs[target_node][a] for a in ('x', 'y', 'z')]

                        la = neuroglancer.LineAnnotation(id=pos, point_a = source_loc, point_b = target_loc)
                        s.layers[f'{syn_id}, {sp_dist}nm'].annotations.append(la)

            json_state_for_seeing_paths = viewer.state.to_json()

            with open(f'{save_dir}/{agglo_id}_partner_distances_along_{self_structure}_to_{soma_target_type}_neuroglancer_state.json', 'w') as fp:
                json.dump(json_state_for_seeing_paths, fp)
...
def save_ng_state_of_sampled_points(em, agglo_seg, seg_id, this_seg_data, results_dir, skel_graph = None):

    all_sim_partners = this_seg_data['simulation_all_sampled_partners']
    edge_coords = this_seg_data['edge_coords']
    ...
    with viewer.txn() as s:
        ...
        s.position = np.mean(np.mean(edge_coords, axis=0), axis=0)

        s.layers['EM'] = neuroglancer.ImageLayer(source=em)
        s.layers['simulated neurite'] = neuroglancer.SegmentationLayer(source=agglo_seg, segments=[seg_id])

        for dtype in ('shaft', 'stalk'):

            for syn_id in this_seg_data[dtype]['simulated_partners']:

                this_syn_point_annos = []
                
                for pos, sim_syn in enumerate(this_seg_data[dtype]['simulated_partners'][syn_id]):

                    pa = neuroglancer.PointAnnotation(id=f'{syn_id}_{pos}', description=sim_syn['partner_id'], point=sim_syn['point'])
                    this_syn_point_annos.append(pa)

                s.layers[f'sim synapses id {syn_id}'] = neuroglancer.AnnotationLayer(annotations=this_syn_point_annos)
                s.layers[f'sim synapses id {syn_id}'].annotationColor = 'white'
            
        all_real_partners = [a for b in [[x['partner_id'] for x in this_seg_data[dtype]['real_partners']] for dtype in ('shaft', 'stalk')] for a in b]

        main_sim_partner = Counter(all_sim_partners).most_common()[0][0]
        main_sim_partner_count = Counter(all_sim_partners).most_common()[0][1]
        main_real_partner = Counter(all_real_partners).most_common()[0][0]
        main_real_partner_count = Counter(all_real_partners).most_common()[0][1]
        
        s.layers[f'greatest real partner count {main_real_partner_count}'] = neuroglancer.SegmentationLayer(source=agglo_seg, segments = [main_real_partner])
        s.layers[f'greatest simluated partner count {main_sim_partner_count}'] = neuroglancer.SegmentationLayer(source=agglo_seg, segments = [main_sim_partner])

        s.layers['simulated neurite simulated location'] = neuroglancer.AnnotationLayer()
        
        for pos, edge in enumerate(edge_coords):
            la = neuroglancer.LineAnnotation(id=pos, point_a = edge[0], point_b = edge[1])
            s.layers['simulated neurite simulated location'].annotations.append(la)

        # Plot skeleton components:
        if skel_graph != None:

            synapse_nodes = [[n['x'],n['y'],n['z']] for n in skel_graph.vs if n['ntype'] == 's' or n['ntype'] == 'sr']
            shaft_nodes = [[n['x'],n['y'],n['z']] for n in skel_graph.vs if n['ntype'] == 'b' or n['ntype'] == 'sr' or n['ntype'] == 'r']
            root_nodes = [[n['x'],n['y'],n['z']] for n in skel_graph.vs if n['ntype'] == 'sr' or n['ntype'] == 'r']
            stalk_nodes = [[n['x'],n['y'],n['z']] for n in skel_graph.vs if n['ntype'] == 't']

            all_locs = [
                ['synapse nodes', synapse_nodes, 'blue'],
                ['shaft nodes', shaft_nodes, 'white'],
                ['root nodes', root_nodes, 'red'],
                ['tb stalk nodes', stalk_nodes, 'green'],
            ]

            for l_name, point_list, col in all_locs:
            
                point_annos = [neuroglancer.PointAnnotation(id=pos, description=pos, point=point) for pos, point in enumerate(point_list)]
                s.layers[l_name] = neuroglancer.AnnotationLayer(annotations=point_annos)
                s.layers[l_name].annotationColor = col


    json_state_for_seeing_points = viewer.state.to_json()

    with open(f'{results_dir}/sim_gp_{main_sim_partner_count}_real_gp_{main_real_partner_count}_neurite_{seg_id}_neuroglancer_state.json', 'w') as fp:
