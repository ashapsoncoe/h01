# stops warning messages appearing
...
# param: points - set of points to fit a circle to
# param: fix_center - if not none, circle will be fit with a center point
...
def plot_all_data_circles(upper_circles, lower_circles, inlier_custers):
...
def main():
    ...
    pd.DataFrame(grid.cv_results_).to_csv(
        f"{top_dir}/cortical_layer_id_5_fold_gaussian_cv_results.csv"
    )
    ...
    for mcs in range(50, 60, 1):
        ...
        clusterer = hdbscan.HDBSCAN(
            min_samples=min_pts, min_cluster_size=mcs, gen_min_span_tree=True
        )
    ...
    for i, cluster in enumerate(clusters):
        ...
        # account for first cluster being less dense
        alpha = 80 if i == 0 else 30
        ...
        with open("./cluster_1_xy.pkl", "wb") as f:
            pickle.dump(np_cluster, f)
    ...
    with open("./conical_bounds_final.json", "w") as f:
        json.dump(bounds, f)

    plot_all_data_circles(upper_bounds, lower_bounds, inlier_clusters)
