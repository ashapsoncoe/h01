...
from common_functions_h01 import get_info_from_bigquery, get_combined_synapses_for_a_seg, assign_root_and_bb_nodes, make_skel_graphs_batch, get_skel_nodes_closest_to_synapses
...
input_file = '001_axons_pure_20_outputs.json' 
skel_dir = 'phase1_001_20percent_skeletons_with_predictions/' ### available at gs://h01_paper_public_files/phase1_001_20percent_skeletons_with_predictions
syn_merge_file = '001_synapse_merge_predictions_20200719.json' ### available at gs://h01_paper_public_files/001_synapse_merge_predictions_20200719.json
pr_data_dir = '001_pr_axons' ### Needs to be unzipped first
...
def get_ground_truth(pr_data_dir, segs):
    ...
    for f in files_todo:
        ...
        results = get_info_from_bigquery(required_info, f'{bq_key}.neuron_id', [seg_id], syn_db_name, client)
        ...
        combined_syn_data = get_combined_synapses_for_a_seg(seg_id, seg_syn_data, syn_merge_d)
...
def test_root_and_bb_identification(seg_ids, max_stalk_len, lower_min, higher_min, min_branch_len, s_model=None):
    ...
    for seg_id in seg_ids:
        ...
        g_temp = assign_root_and_bb_nodes(g_temp, max_stalk_len, lower_min, higher_min, min_branch_len, nodes_or_lengths)
...
if __name__ == '__main__':
    ...
    pr_data_dir = f'{working_dir}/{pr_data_dir}'
    ...
    for i in range(10000):
        skel_graphs_batch = make_skel_graphs_batch(i, organized_neurites[i], skel_dir, skel_voxel_size, credentials_file, neurite_type, syn_db_name, syn_location_d=gt_syn_only)
    ...
    for seg_id in neurites_todo:
        for syn_id in ground_truth[seg_id]['synapses'].keys():
            if ground_truth[seg_id]['synapses'][syn_id]['syn_type'] == 'stalk':
                ...
                selected_node = get_skel_nodes_closest_to_synapses([loc], skel_graphs[seg_id], [n.index for n in skel_graphs[seg_id].vs])[0]
    ...
    for seg_id in ground_truth.keys():
        ...
        end_nodes = get_skel_nodes_closest_to_synapses(end_points, skel_graphs[seg_id], [n.index for n in skel_graphs[seg_id].vs])
        all_bb_nodes = set()
    ...
    for seg_id in skel_graphs_temp.keys():
        skel_graphs_temp[seg_id] = assign_root_and_bb_nodes(skel_graphs_temp[seg_id], max_stalk_len, lower_min, higher_min, min_branch_len, nodes_or_lengths)
    ...
    with open(f'{working_dir}/{neurite_type}_skeleton_pruning_parameters.json', 'w') as fp:
        json.dump(chosen_parameters, fp)

    with open(f'{working_dir}/{neurite_type}_skeleton_pruning_test_performance.json', 'w') as fp:
        json.dump(performance_summary, fp)

    with open(f'{working_dir}/{neurite_type}_synapse_classification_model.pkl', 'wb') as fp:
        pickle.dump(clf, fp)
